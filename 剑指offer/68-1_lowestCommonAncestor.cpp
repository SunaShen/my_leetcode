// @File   : 68-1_lowestCommonAncestor.cpp
 ...
 *
 * 1.利用二叉搜索树性质
 *  若当前节点值大于最大节点值，则搜索左子树
 *  若当前节点值小于最小节点值，则搜索右子树
 *  若当前节点处于两者之间，则该节点为公共节点。
 *
 *
 */
...
public:
    TreeNode* lowestCommonAncestor(TreeNode* root, TreeNode* p, TreeNode* q) {
        ...
        while(node){
            int n = node->val;
            if(n<min_val) node = node->right;
            else if(n>max_val) node = node->left;
            else return node;
        }
        return nullptr;
    }
